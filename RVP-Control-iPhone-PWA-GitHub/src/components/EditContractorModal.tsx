...
import { updateContractor, fetchRegions, type ContractorListItem, type ContractorStatus, type RegionOption } from '@/lib/contractorsApi';
...
export default function EditContractorModal({ contractor, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast('Дані підрядника оновлено', 'success');
      onSaved();
      onClose();
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Помилка оновлення підрядника';
      showToast(msg, 'error');
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#141720] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-[#141720] border-b border-white/5 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-base font-semibold text-white">Редагувати підрядника</h2>
            <p className="text-xs text-slate-500 mt-0.5">{contractor.name}</p>
          </div>
