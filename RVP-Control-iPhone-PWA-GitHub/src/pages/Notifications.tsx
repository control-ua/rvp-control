...
export default function Notifications() {
  ...
  return <PageHeader pageTitle="Сповіщення" pageSubtitle={`${unread} непрочитаних`}
    ...
    <div className="overflow-hidden rounded-2xl border border-white/5 bg-[#141821]">
      ...
      filtered.map(item=><button key={item.id} onClick={()=>setItems(p=>p.map(x=>x.id===item.id?{...x,read:true}:x))} className="flex w-full items-start gap-4 border-b border-white/5 px-5 py-4 text-left last:border-0 hover:bg-white/[0.025]">
